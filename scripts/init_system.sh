#/bin/bash
# Copyright 2020 The dustess Authors.
#
...
if [ $? -eq 0 ]; then
   ...
   echo 'br_netfilter' > /etc/modules-load.d/cf-br_netfilter.conf
fi

modinfo overlay > /dev/null 2>&1
if [ $? -eq 0 ]; then
   modprobe overlay
   echo 'overlay' > /etc/modules-load.d/cf-br_netfilter.conf
fi
...
#sed -i ':a;$!{N;ba};s@# cf hosts BEGIN.*# cf hosts END@@' /etc/hosts
sed -i '/^$/N;/\n$/N;//D' /etc/hosts
